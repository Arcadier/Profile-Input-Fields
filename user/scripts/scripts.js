 ...
 * @type {string}
 */
const tableName = "customFieldsUserData";
 ...
 * @type {string}
 */
const packageId = "456a3ff5-bb72-4a0a-93f3-1743b50764c9";

$(document).ready(function () {
    // checks if the page is right by checking against the body's class
    if (document.body.className == "page-seller seller-items page-settings pace-running") {
        ...
        document.getElementsByClassName("my-btn btn-red")[1].addEventListener("click", saveCustomFields);
        ...
        // if the type of the input is selector
          ...
          <select class="required" id="custom-field-${heading}" name="${heading}"></select></div>`)[0];
            ...
            for (var j = 0; j < options.length; j++) {
                ...
                var optionTag = $.parseHTML(`<option value="${currOption}">${currOption}</option>`)[0];
...
function addRowCustomTable(userId, inputJson, customTableName) {
    ...
    var settings = {
        "url": "https://" + baseUrl + "/api/v2/plugins/" + packageId + "/custom-tables/" + customTableName + "/rows",
        "method": "POST",
        "headers": {
            "Content-Type": "application/json"
        },
...
function editRowCustomTable(rowId, inputJson, customTableName) {
    ...
    var settings = {
        "url": "https://" + baseUrl + "/api/v2/plugins/" + packageId + "/custom-tables/" + customTableName + "/rows/" + rowId,
        "method": "PUT",
        "headers": {
            "Content-Type": "application/json"
        },
